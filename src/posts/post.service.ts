...
export class PostService {
  ...
  async createPost(dto: {
    ...
    const user = await this.usersService.findUserByIdentifier({
      id: dto.senderId,
    });
    if (!user) {
    ...
    this.logger.log(
      `User ${user._id} is creating a post in campus ${dto.campusId}: ${JSON.stringify(user.campus)}`
    );

    const userCampusId =
      typeof user.campus === "object" && "id" in user.campus
