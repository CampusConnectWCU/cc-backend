...
export class UsersService {
  ...
  async validateUserCredentials(dto: LoginDto): Promise<IUser> {
    ...
    this.logger.log(`User found: ${user ? user.username : "none"}.`);
    if (!user) {
  ...
  async updateUser(userId: string, updateData: UpdateUserDto): Promise<IUser> {
    ...
    const newPasswordProvided =
      updateData.password && updateData.password.trim().length > 0;
    const oldPasswordProvided =
      updateData.oldPassword && updateData.oldPassword.trim().length > 0;
    ...
    if (newPasswordProvided) {
      ...
      const isOldPasswordCorrect = await this.comparePasswords(
      ...
      this.logger.warn(
        `User ${userId} provided oldPassword without a valid new password during update.`
      );
      throw new BadRequestException(
        "New password cannot be empty when changing password."
      );
    }
    ...
    if (
      ...
      const existingUser = await this.usersRepository.findByIdentifier({
    ...
    if (updateData.campusId !== undefined) {
      const campusExists = await this.campusService.findCampusById(
        updateData.campusId
      );
      if (!campusExists) {
      ...
      if (user.campus) {
        if (typeof user.campus === "string") {
          ...
          typeof user.campus === "object" &&
          (user.campus as any)._id
        ) {
    ...
    if (Object.keys(payloadToUpdate).length === 0) {
      this.logger.log(
        `No changes detected for user ${userId}. Returning current user data.`
      );
      // Re-fetch even if no changes to ensure consistent return structure (populated)
      const finalUser = await this.usersRepository.findByIdentifier({
        id: userId,
      });
      if (!finalUser) {
    ...
    this.logger.log(
      `Updating user ${userId} with fields: ${JSON.stringify(Object.keys(payloadToUpdate))}`
    );
    // Perform the update
    ...
    this.logger.log(
      `Re-fetching user ${userId} with populated fields after update.`
    );
    const updatedPopulatedUser = await this.usersRepository.findByIdentifier({
      id: userId,
    });
