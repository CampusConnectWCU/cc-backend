...
export class ChannelsService {
  ...
  async getOrCreateDMChannel(
    ...
    const existing = await this.channelsRepository.findDMChannel(
      userId1,
      userId2
    );
    ...
    this.logger.log(
      `Creating new DM channel between ${user1.username} and ${user2}.`
    );
  ...
  async getChannelsMeta(userId: string) {
    ...
    return Promise.all(
      channels.map(async (ch) => {
        ...
        const unread = await this.messageRepository.countUnread(
          channelId,
          userId
        );
  ...
  async searchChannels(userId: string, query: string) {
    ...
    return meta
      .map((m) => {
        ...
        if (
          m &&
          m.participants.some((p) => /* lookup username/fullname */ false)
        )
          score += 50;
